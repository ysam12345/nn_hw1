...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        # 設定訓練圖表
        self.training_figure = Figure(figsize=(3,3), dpi=100)
        self.training_canvas = FigureCanvasTkAgg(self.training_figure, self)
        self.training_canvas.draw()
        self.training_canvas.get_tk_widget().grid(row=3, columnspan=3)
        ...
        self.weight_text["width"] = 30
        self.weight_text.grid(row=6, column=1, sticky=tk.N+tk.W)

    def draw_training_figure(self, train_result_list):
        #清空影像
        self.training_figure.clf()
        self.training_figure.a = self.training_figure.add_subplot(111)
        acc_list = [i['acc'] for i in train_result_list]
    
        #繪製正確率折線圖
        self.training_figure.a.plot(acc_list)
        self.training_figure.a.set_title('Traing: Acc/Epoch')
        self.training_canvas.draw()
    ...
    def select_file_and_run(self):
        ...
        for i in range(int(self.epoch_spinbox.get())):
            ...
            if train_result["acc"] > int(self.early_stop_spinbox.get()):
                break
        print("### training end ###")
        self.draw_training_figure(train_result_list)
